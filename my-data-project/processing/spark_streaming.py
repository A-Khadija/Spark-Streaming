...
from pyspark.sql.functions import from_json, col, struct, count, window, split, dayofweek, to_timestamp
from pyspark.sql.types import StructType, StructField, StringType, IntegerType, FloatType
import redis

# --- CONFIGURATION ---
KAFKA_BOOTSTRAP = "kafka:29092"   # Connects to the internal Docker network listener
TOPIC = "user_behavior"
...
def process_stream():
    ...
    postgres_query = transformed_stream.writeStream \
        .foreachBatch(lambda df, epoch_id: df.write.jdbc(url=POSTGRES_URL, table="raw_events", mode="append", properties=POSTGRES_PROPERTIES)) \
    ...
    query_activity = user_activity.writeStream \
        .outputMode("complete") \
        .foreachBatch(write_features_to_redis) \
        .trigger(processingTime='5 seconds') \
        .start()
